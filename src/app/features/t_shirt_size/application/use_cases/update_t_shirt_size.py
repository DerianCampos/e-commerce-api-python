...
class UpdateTShirtSizeUseCase:
    ...
    async def execute(self, size_id: str, size_update: TShirtSizeUpdateRequest) -> TShirtSizeResponse:
        ...
        try:
            ...
            temp_entity = TShirtSizeEntity.__new__(TShirtSizeEntity)
            temp_entity._id = size_entity_id
            temp_entity._size = size
            temp_entity._chest_min_cm = size_update.chest_min_cm
            temp_entity._chest_max_cm = size_update.chest_max_cm

            updated_size = await self.size_repository.update(temp_entity)
