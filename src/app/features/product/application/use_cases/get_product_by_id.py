...
class GetProductByIdUseCase:
    ...
    async def execute(self, product_id: str) -> ProductResponse:
        try:
            ...
            product_obj_id = EntityId(product_uuid)

            existing_product = await self.product_repository.find_by_id(product_obj_id)

            if not existing_product:
                log.warning(f"Product not found with ID: {product_id}")
                raise ProductDoesNotExistException(product_obj_id)
