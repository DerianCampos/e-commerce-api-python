...
async def get_user_repository(session: AsyncSession = Depends(get_database_session)) -> UserRepositoryImpl:
    return UserRepositoryImpl(session)


async def get_product_repository(session: AsyncSession = Depends(get_database_session)) -> ProductRepositoryImpl:
    return ProductRepositoryImpl(session)


async def get_band_repository(session: AsyncSession = Depends(get_database_session)) -> BandRepositoryImpl:
